<?php echo $__env->make('layouts.header', ['page_title' => 'Training Materials List'], array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
  <!-- Page -->
  <div class="page">
              ...
              <div class="card card-shadow card-responsive" id="widgetTimeline">
                <div class="card-block p-0">
                  ...
                  <table class="table table-hover tabletable-striped dataTable" id="data_table">
                      ...
                      <tbody> 
						  <?php $sno = 1; ?>
                            <?php $__currentLoopData = $training_materials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $training_materials): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>                            
                            <tr>
								<td><?php echo e($sno++); ?></td>
                                <td><?php echo e($training_materials->training_name); ?></td>
                                <td><?php if($training_materials->reading_type==0): ?> <?php echo e('Pre Reading'); ?> <?php elseif($training_materials->reading_type==1): ?> <?php echo e('Post Reading'); ?><?php endif; ?></td>
                                <td><?php echo e($training_materials->training_material_name); ?></td>
                                <td><?php if(isset($training_materials) && $training_materials->training_material!='' && file_exists(public_path('uploads').'/'.$training_materials->training_material)): ?> <a href="public/uploads/<?php echo $training_materials->training_material; ?>" target="_blank"><?php echo e($training_materials->training_material); ?></a> <?php endif; ?></td>
                                <td class="actions" id="status_div_<?php echo e($training_materials->id); ?>"><?php if($training_materials->status==0): ?> <a onclick="fnStatusChange(<?php echo e($training_materials->id); ?>,<?php echo e($training_materials->status); ?>)"><i class="btn btn-sm btn-warning waves-effect icon fa-eye-slash" aria-hidden="true" title="Make Active"></i></a> <?php else: ?> <a onclick="fnStatusChange(<?php echo e($training_materials->id); ?>,<?php echo e($training_materials->status); ?>)"><i class="btn btn-sm btn-success waves-effect icon fa-eye" aria-hidden="true" title="Make Inactive" ></i></a> <?php endif; ?></td>
                                <td class="actions"><a onClick="fnEditTrainingMaterial(<?php echo e($training_materials->id); ?>)"><i class="btn btn-sm btn-primary waves-effect icon fa-edit" aria-hidden="true" title="Edit"></i></a><a onClick="fnDelete(<?php echo e($training_materials->id); ?>)"><i class="btn btn-sm btn-danger waves-effect icon fa-close" aria-hidden="true"  title="Delete"></i></a></td>
                            </tr>                            
            ...
            <div class="col-xs-12">
                ...
                <select class="form-control" id="training_id" name="training_id">
                  <option value = ''>Select</option>
                    <?php $__currentLoopData = $trainings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $training): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>  
                      <?php if($training->training_name!='' && $training->id!=0): ?>
                        <option value = "<?php echo e($training->id); ?>" <?php echo e(((old("training_id")!='' && old("training_id") ==  $training->id)) ? "selected=selected":""); ?>><?php echo e($training->training_name); ?>
              ...
              <div class="form-group">
                ...
                <select class="form-control" id="reading_type" name="reading_type">
                  <option value = ''>Select</option>
                  <option value = "0" <?php echo e(((old("reading_type")!='' && old("reading_type") ==  0)) ? "selected=selected" :""); ?>>Pre Reading
                  </option>  
                  <option value = "1" <?php echo e(((old("reading_type")!='' && old("reading_type") ==  1)) ? "selected=selected" :""); ?>>Post Reading
                  </option>               
                  </select> 
              </div>  
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="training_material_name" name="training_material_name" placeholder="Training Material Name" value="<?php echo e(old('training_material_name')!='' ? old('training_material_name') : ''); ?>">
              </div> 
