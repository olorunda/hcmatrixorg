...
$(function(){	
	...
	****/
	...
	$('#jobrole').change(function(){
			...
			$.get('<?php echo e(url('getleaveday')); ?>',{
				jobrole:jobrole
				...
				if(xhr.status==200){
					...
					$('#leaveday').val(data);
						...
						$('#saveattach').click(function(){
							...
							$.get('<?php echo e(url('attachleave/role')); ?>',{
								jobrole:jobrole,
								leaveday:leaveday
	...
	***/
	...
	url: "<?php echo e(url('import/emloyee')); ?>",
	maxFilesize: 1,
	parallelUploads:1,
	acceptedFiles:'.csv',
	maxFiles:1,
  ...
  $('#importemployee').click(function(){
	...
	***/
	...
	$('#modifyqtemplate').click(function(){
	 ...
	 $.post('<?php echo e(url('modify')); ?>/query',{
		 template:template,
		 title:title,
		 qtypeid:qtypeid,
		 _token:token
	...
	$('#modifyqtemplatel').click(function(){
	 ...
	 $.post('<?php echo e(url('modify')); ?>/leave',{
		 daynum:templatel,
		 title:titlel,
		 qtypeidl:qtypeidl,
		 _token:token
	...
	$('#savemodhols').click(function(){
		...
		holid=$('#qtype1lp').val();
		token=$('#token').val();
		$.post('<?php echo e(url('mod/holiday')); ?>',{
	...
	$('#savehols').click(function(){
		...
		$.get('<?php echo e(url('save/holiday')); ?>?holname='+holname+'&holfrom='+holfrom+'&holto='+holto,function(data,status,xhr){
	...
	//
		$('#actiontype').change(function(){
						...
						$('#getfiscal').click(function(){
							$.get('<?php echo e(url('getfiscal')); ?>',function(data,status,xhr){
						...
						$('#savefiscal').click(function(){
							...
							startmont=$('#startmont').val();
								...
								$.post('<?php echo e(url('savefiscal')); ?>',{
