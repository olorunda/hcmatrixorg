@include('layouts.header', ['page_title' => 'My Expenses'])
  <!-- Page -->
  <div class="page">
                  ...
                  <table class="table table-hover tabletable-striped dataTable" id="data_table">
                      ...
                      <tbody> 
                            ...
                            <tr>                                
                                ...
                                <td class="actions">
                                  @if($expense->expense_status==Config::get('constants.expense_status.APPLIED')) <span class="tag tag-pill tag-primary">Applied</span> @endif
                                  @if($expense->expense_status==Config::get('constants.expense_status.APPROVED')) <span class="tag tag-pill tag-success">Approved</span> @endif
                                  @if($expense->expense_status==Config::get('constants.expense_status.REVISE')) <span class="tag tag-pill tag-warning">Revise</span> @endif
                                  @if($expense->expense_status==Config::get('constants.expense_status.REVISED')) <span class="tag tag-pill tag-info">Revised</span> @endif
                                  @if($expense->expense_status==Config::get('constants.expense_status.REJECTED')) <span class="tag tag-pill tag-danger">Rejected</span> @endif
                                </td>
                                <td class="actions">@if($expense->expense_status!=Config::get('constants.expense_status.APPROVED') && $expense->expense_status!=Config::get('constants.expense_status.REJECTED'))<a onClick="fnEditExpense({{$expense->id}})"><i class="btn btn-sm btn-primary waves-effect icon fa-edit" aria-hidden="true" title="Edit"></i></a>@endif @if($expense->expense_status!=Config::get('constants.expense_status.APPROVED'))<a onClick="fnDelete({{$expense->id}})"><i class="btn btn-sm btn-danger waves-effect icon fa-close" aria-hidden="true" title="Delete"></i></a>@endif</td>
                            </tr>                            
                            @endforeach 
                      </tbody> 
                  </table>           
