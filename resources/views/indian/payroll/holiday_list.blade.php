@include('layouts.header', ['page_title' => 'Holiday List'])
  <!-- Page -->
  <div class="page">
                  ...
                  <table class="table table-hover tabletable-striped dataTable" id="data_table">
                      ...
                      <tbody> 
                          <?php $sno = 1; ?>
                            ...
                            <tr>  
                                <td>{{$sno++}} </td>                             
                                <td>@if($holiday->multiple_days==1) {{ date("M d, Y", strtotime($holiday->from_date)) }} to {{ date("M d, Y", strtotime($holiday->to_date)) }} @else {{ date("M d, Y", strtotime($holiday->single_day)) }} @endif</td>
                                <td><p>{{ $holiday->reason }}</p></td>
                                <td class="actions" id="status_div_{{ $holiday->id}}">@if($holiday->status==0) <a onclick="fnStatusChange({{ $holiday->id}},{{ $holiday->status}})"><i class="btn btn-sm btn-warning waves-effect icon fa-eye-slash" aria-hidden="true"   title="Make Active"></i></a> @else <a onclick="fnStatusChange({{ $holiday->id}},{{ $holiday->status}})"><i class="btn btn-sm btn-success waves-effect icon fa-eye" aria-hidden="true" title="Make Inactive"></i></a> @endif</td>
                                <td class="actions"><a onClick="fnEditHoliday({{$holiday->id}})"><i class="btn btn-sm btn-primary waves-effect icon fa-edit" aria-hidden="true" title="Edit"></i></a><a onClick="fnDelete({{ $holiday->id}})"><i class="btn btn-sm btn-danger waves-effect icon fa-close" aria-hidden="true" title="Delete"></i></a></td>
                            </tr>                            
                            @endforeach 
                      </tbody> 
                  </table>           
              ...
              <div class="form-group" id="single_day_div">
                ...
                <div class="">
                  <div class="input-group">
                    ...
                    <input type="text" class="datepicker form-control" id="single_day" name="single_day" placeholder="Day" value="{{ old('single_day')!='' ? old('single_day') : ((isset($holiday_details) && $holiday_details->single_day!='') ? date('M d, Y', strtotime($holiday_details->single_day)) : '') }}" data-plugin="datepicker">
                  </div>
                </div>  
              </div> 
