@include('layouts.header', ['page_title' => 'Leave List'])
  <!-- Page -->
  <div class="page">
                  ...
                  <table class="table table-hover tabletable-striped dataTable" id="data_table">
                      ...
                      <tbody> 
                            ...
                            <tr>                                
                                ...
                                <td class="actions">@if($leave->leave_status==Config::get('constants.apply_leave_status.APPLIED') && (strtotime($leave->from_date)>strtotime(date("Y-m-d"))))<a onClick="fnCancel({{ $leave->id}})"><i class="btn btn-sm btn-danger waves-effect icon fa-close" aria-hidden="true" title="Cancel"></i></a>@endif</td>
                            </tr>                            
                            @endforeach 
                      </tbody> 
                  </table>           
          ...
          <div class="row row-lg col-xs-12">            
            ...
            <div class="form-group">
                <div class="col-xs-3 no-padding">  
                  <label class="example-title">Leave Type&nbsp;<span class="required_filed">*</span></label>
                </div>
                <div class="col-xs-9">  
                  <input type="radio" name="is_casual_leave" value="1">&nbsp;Casual Leave&nbsp;&nbsp;&nbsp;&nbsp;
                  <input type="radio" name="is_casual_leave" value="0">&nbsp;Non-casual Leave
                  <div id="is_casual_leave_err" class="col-xs-10 no-padding"></div> 
                </div>     
              </div> 
                ...
                <div class="example">
                  <div class="input-daterange" data-plugin="datepicker">
                    <div class="input-group">
                      ...
                      <input type="text" class="form-control" name="from_date" id="from_date" value="{{ old('from_date')!='' ? old('from_date') : ((isset($leave_details) && $leave_details->from_date!='') ? date('M d, Y', strtotime($leave_details->from_date)) : '') }}" />                      
                    </div>                    
                    <div class="input-group">
                      <span class="input-group-addon">to</span>
                      <input type="text" class="form-control" name="to_date" id="to_date" value="{{ old('to_date')!='' ? old('to_date') : ((isset($leave_details) && $leave_details->to_date!='') ? date('M d, Y', strtotime($leave_details->to_date)) : '') }}"  />                      
                    </div>                   
                  </div>                   
                </div>
              ...
              <div class="form-group">
                ...
                <textarea class="form-control" id="leave_comment" name="leave_comment" placeholder="Comment">{{ old('leave_comment')!='' ? old('leave_comment') : ((isset($leave_details) && $leave_details->leave_comment!='') ? $leave_details->leave_comment : '') }}</textarea>               
              </div>
...
"aoColumnDefs": [
  ...
  {
    var html = '';
    var is_casual_leave = $("input[name$='is_casual_leave']:checked").val();
    var from_date = '';
    var to_date = '';
    if($("#from_date").val())
    ...
    //If casual leave
    if(is_casual_leave==1 && from_date!='' && to_date!='')
    {
      $.ajax({
          type: "POST",
          url: "getavailleave",
  ...
  $("input[name$='is_casual_leave']").click(function()
  { 
    date_change();
  });
  ...
  function fnLeave()
    {
      ...
      $.ajax({
        ...
        if(errors.is_casual_leave)
        {
          $("#is_casual_leave_err").html('<div class="flash-message"><div class="alert alert-danger"><strong>'+errors.is_casual_leave+'</strong></div></div>');
        }
        if(errors.from_date)
        {
