...
<head>
 ...
 @if(active(session('locale').'/employee/profile_*'))
  ...
  <link rel="stylesheet" type="text/css" href="{{asset('assets/css/typeaheadjs.css')}}">
...
<link rel="stylesheet" type="text/css" href="{{asset('assets/css/sweetalert.css')}}">
  ...
  <link rel="stylesheet" href="{{asset('assets/css/fullcalendar.min.css')}}">
	     @if(active(session('locale').'/manage/*') || active(session('locale').'/leave') || active(session('locale').'/sort/*') || active(session('locale').'/search'))
  ...
  @elseif(active(session('locale').'/lm/goals'))
  <link rel="stylesheet" href="{{asset('global/vendor/summernote/summernote.css')}}">
  <link rel="stylesheet" href="{{asset('global/vendor/select2/select2.css')}}">
  ...
  @elseif(active(session('locale').'/employee/absence') || active(session('locale').'/manage/*'))
  <link rel="stylesheet" href="{{asset('global/vendor/select2/select2.css')}}">
  <link rel="stylesheet" href="{{asset('global/vendor/chartist/chartist.css')}}">
  ...
  <style>
  @if(active(session('locale').'/view/attendancecalender'))
  .fc-content{
 ...
 .fc-list-heading-alt,.fc-list-heading-main{
  ...
  @if(active(session('locale').'/view/docadmin')|| active(session('locale').'/view/mydocument'))
  div.raspberry {
    float: left;
    margin: 2px;
}
...
@if(active(session('locale').'/applicant/job'))
@else	
	  margin-top: @if(active(session('locale').'/employee/list')) -25px @elseif(active(session('locale').'/manage/*')||active(session('locale').'/leave') ||active(session('locale').'/lm/*')|| active(session('locale').'/search'))  @else -40px; @endif @endif
  }
  .pdfSlider_close{
...
@if(active(session('locale').'/manage/absence') || active(session('locale').'/leave') || active(session('locale').'/sort/*') || active(session('locale').'/search')|| active(session('locale').'/applicant/*'))
	<body class="animsition app-contacts page-aside-left site-menubar-fold"   >
@elseif(active(session('locale').'/lm/query')|| active(session('locale').'/lm/querytype/*') || active(session('locale').'/employee/list')|| active(session('locale').'/employee/linemanager'))
...
@endif
      ...
      <div  class="collapse navbar-collapse navbar-collapse-toolbar" id="site-navbar-collapse">
        ...
        <ul class="nav navbar-toolbar">
          ...
          <li class="nav-item hidden-float">
            ...
            role="button">
              <span class="sr-only">{{_t('Toggle Search')}}</span>
            </a>
          </li>
        </ul>
        <!-- End Navbar Toolbar -->
        ...
        <ul class="nav navbar-toolbar navbar-right navbar-toolbar-right">
          ...
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu" role="menu">
              <a class="dropdown-item" href="{{url('employee/profile')}}" role="menuitem"><i class="icon wb-user" aria-hidden="true"></i> {{_t('Profile')}}</a>
              <a class="dropdown-item" href="javascript:void(0)" role="menuitem"><i class="icon wb-settings" aria-hidden="true"></i> {{_t('Settings')}}</a>
              <div class="dropdown-divider" role="presentation"></div>
              <a class="dropdown-item" href="{{url('logout')}}" role="menuitem" ><i class="icon wb-power" aria-hidden="true"></i> {{_t('Logout')}}</a>
      ...
      <div>
        <div>
          <ul class="site-menu" data-plugin="menu">
            <li class="site-menu-category">{{_t('Welcome :name',['name'=>Auth::user()->name])}}</li>
  ...
  <!-- Page -->
@if(active(session('locale').'/manage/absence') || active(session('locale').'/leave') || active(session('locale').'/sort/*') || active(session('locale').'/search')|| active(session('locale').'/applicant/*') || active(session('locale').'/lm/query')||active(session('locale').'/lm/querytype/*') || active(session('locale').'/employee/list')||active(session('locale').'/employee/linemanager'))
	  		@yield('content')
  ...
  <footer class="site-footer">
	...
	{{_t('Developed By ')}} <a href="http://www.snapnet.com.ng">Snapnet Limited</a>
    </div>
  </footer>
  <!-- Core  -->
  ...
  <script src="{{asset('global/vendor/tether/tether.js')}}"></script>
   @if(active(session('locale').'/employee/absence') || active(session('locale').'/manage/*') || active(session('locale').'/applicant/*'))
		@if(active(session('locale').'/manage/absence'))
  ...
  <script src="{{asset('global/vendor/bootstrap/bootstrap.js')}}"></script>
  @endif
  @if(active(session('locale').'/manage/leavestat') || active(session('locale').'/manage/positions'))
	    <script src="{{asset('global/vendor/bootstrap/bootstrap.js')}}"></script>
  ...
  @elseif(active(session('locale').'/lm/objectives_c')||active(session('locale').'/employee/*'))
  ...
  <script src="{{asset('assets/js/myrate.js')}}"></script>
    @elseif(active(session('locale').'/job/*'))
      <script src="{{asset('global/vendor/select2/select2.full.min.js')}}"></script>
      <script src="{{asset('global/js/Plugin/select2.js')}}"></script>
 ...
 @if(active(session('locale').'/manage/absence') || active(session('locale').'/leave') || active(session('locale').'/sort/*') || active(session('locale').'/search')|| active(session('locale').'/applicant/*'))
  ...
  @if(active(session('locale').'/manage/*')  || active(session('locale').'/sort/*') )

@if(active(session('locale').'/manage/absence') || active(session('locale').'/leave') || active(session('locale').'/search'))
	@else
  <script src="{{asset('assets/js/Site.js')}}"></script>
  <script src="{{asset('global/js/Plugin/asscrollable.js')}}"></script>
  <script src="{{asset('global/js/Plugin/slidepanel.js')}}"></script>
  ...
  <script src="{{asset('global/js/Plugin/selectable.js')}}"></script>
  <script src="{{asset('global/js/Plugin/bootbox.js')}}"></script>
  <script src="{{asset('global/js/jqextendputdelete.js')}}"></script>
  <script src="{{asset('assets/js/BaseApp.js')}}"></script>
  <script src="{{asset('assets/js/App/Contacts.js')}}"></script>
  <script src="{{asset('assets/examples/js/apps/contacts.js')}}"></script>
  @endif
 ...
 @if( active(session('locale').'/manage/*') )
	 @else
      <script src="{{asset('global/vendor/bootstrap/bootstrap.js')}}"></script>
  @endif
      <script src="{{asset('global/vendor/select2/select2.full.min.js')}}"></script>
  ...
  @endif
	@if(active(session('locale').'/manage/leavestat'))
		  <script src="{{asset('assets/examples/js/charts/morris.js')}}"></script>
	@endif
  ...
  @if(active(session('locale').'/project/*'))
	  <script src="{{asset('assets/js/bootstrap-typeahead.min.js')}}" type="text/javascript"></script>
  @endif
  @if(active(session('locale').'/lm/goals'))
