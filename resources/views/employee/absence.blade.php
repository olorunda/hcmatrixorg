      ...
      <div class="card-block bg-white p-20">
        ...
        <div class="content-text text-xs-center m-b-0">
          ...
          <span class="font-size-40 font-weight-100">
            @if(count($yearlyDue) > 0)
            {{$yearlyDue->day_num}} Days
            @else
            0
            @endif
          </span>
    ...
    <div class="card card-shadow">
      <div class="card-block bg-white p-20">
        ...
        <div class="content-text text-xs-center m-b-0">
          ...
          <span class="font-size-40 font-weight-100">{{count($requests)}}</span>
  ...
  <div class="col-ms-12 col-xs-12 col-md-6">
    <div class="card card-shadow">
      <div class="card-header card-header-transparent p-y-20">
        <div>
          <input type="hidden" name="_request_token" id="requesttoken" value="{{csrf_token()}}">
          <div class="row">
          ...
          <table class="table table-hover borderless">
            ...
            <tbody>
              ...
              <tr>
                ...
                <th>{!!status($request->lm_approve)!!}</th>
                @if($request->lm_comments==NULL)
                <th>No comments yet.</th>
                @else
                <th>{{$request->lm_comments}}</th>
                @endif
                <th>{!!status($request->admin_approve)!!}</th>
                @if($request->admin_comments==NULL)
                <th>No comments yet.</th>
                @else
                <th>{{$request->admin_comments}}</th>
                @endif
                ...
                <th>
                  ...
                  @elseif($request->lm_approve == 1 || $request->admin_approve == 1 || $request->board_approve == 1 || $request->lm_approve == 2 || $request->admin_approve == 2 || $request->board_approve == 2 || $request->status == 2 )
                  <a class="text-success" href="javascript:void(0)" title="LOCKED. AWAITING FURTHER RESPONSE"><i class="icon wb-lock"></i></a>
                  @else
                  <a class="text-primary" href="javascript:void(0)" data-toggle="modal" data-target="#modify{{$request->id}}" title="MODIFY"><i class="icon wb-edit"></i></a>
                  <a class="text-danger" href="javascript:void(0)" onclick="modifyRequest({{$request->id}}, 2)" title="DELETE"><i class="icon wb-power"></i></a>
                  @endif
                </th>
              </tr>
              ...
              <tr>
                ...
                <th colspan="2">{{$totalreqdays}}</th>
                <input type="hidden" name="totalreq" id="totalreq" value="{{$totalreqdays}}">
              </tr>
            </tbody>
          </table>
        </div>
...
@if(count($requests) > 0)
@foreach($requests as $request)
<div class="modal fade" id="modify{{$request->id}}" aria-hidden="true" aria-labelledby="examplePositionSidebar" role="dialog" tabindex="-1">
  <div class="modal-dialog modal-lg modal-sidebar">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row" style="width: 400px;">
          <div class="form-group col-xs-12 col-md-12">
            ...
            <textarea class="form-control" style="height: 100px;resize: none;" disabled="disabled">{{$request->reason}}</textarea>
          </div>
          <div class="form-group col-xs-12 col-md-12">
          ...
          <div class="form-group col-xs-12 col-md-12">
            ...
            <input type="text" name="modstart{{$request->id}}" id="modstart{{$request->id}}" class="form-control" value="{{$request->startdate}}" autocomplete="off" data-plugin="datepicker" data-date-format="yyyy-mm-dd" required="required">
          </div>
          <div class="form-group col-xs-12 col-md-12">
            <label class="form-control-label">END DATE</label>
            <input type="text" name="modend{{$request->id}}" id="modend{{$request->id}}" class="form-control" value="{{$request->enddate}}" autocomplete="off" data-plugin="datepicker" data-date-format="yyyy-mm-dd" required="required">
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <div class="row" style="width: 400px;">
          <div class="form-group col-xs-12 col-md-12">
            <button type="button" class="btn btn-success btn-block" onclick="modifyRequest({{$request->id}}, 1)">Save changes</button>
          </div>
        </div>
      </div>
    </div>
