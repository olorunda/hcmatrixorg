...
function scorerange(){
	...
	window.location='{{url('applicant')}}/job?id={{session('jobid')}}&type=s&startscore='+startscore+'&endscore='+endscore;
...
$(function(){
	...
	window.location='{{url('applicant')}}/job?id={{session('jobid')}}'+'&type='+jobtype;
		return;
	}
	window.location='{{url('applicant')}}/job?id='+jobtype;
	...
	<div class="page-aside">
		...
		<div class="page-aside-inner page-aside-scroll">
			<div data-role="container">
					...
					<div class="page-aside-section">
						<div class="list-group">
							<a class="list-group-item" href="javascript:void(0)">
								<span class="item-right">{{count($applicants)}}</span><i class="icon wb-inbox" aria-hidden="true"></i>All Applicant(s)
							</a>
				...
				<option disabled>-{{strtoupper('Job Title')}}-</option>
				@if(count($availablejobs)>0)
				@foreach($availablejobs as $jobs)
			    <option value="{{$jobs->id}}">{{strtoupper($jobs->title)}}</option>
			    @endforeach
				@endif
		...
		<div id="contactsContent" class="page-content page-content-table" data-plugin="asSelectable">
			...
			<table class="table table-condensed" id="directempstable">
				...
				<input type="hidden" value="{{csrf_token()}}" id="token" />
					...
					@foreach($applicants as $emp)
					<tr>
						<td >
<?php  $job=app('App\Repositories\EmployeeRepository')->getappdetail($emp->job_app_id) ?>
						<input type="checkbox"  class="appid" name="appid[]" value="{{$emp->id}}" >
						
					
						<input type="hidden"   id="email{{$emp->id}}" value="{{$emp->email}}" >
						<input type="hidden"   id="name{{$emp->id}}" value="{{$emp->name}}" >
						<input type="hidden"   id="job{{$emp->id}}" value="{{$job}}" >
						...
						<td id="stat{{$emp->id}}">@if($emp->status==0) <span class="tag tag-warning">Pending </span> @elseif($emp->status==1) <span class="tag tag-success">Approved </span>@else <span class="tag tag-danger">Rejected </span> @endif</td>
						<td> <p style="margin:0 20px 0 20px;">{{app('App\Repositories\EmployeeRepository')->getscore($emp->id,$emp->job_app_id)}}</p> </td>
						<td height="10">
					  ...
					  @if(count($docs)>0)
						@foreach($docs as $doc)
                        <a data-toggle="modal" onclick="urlChange('{{url('upload')}}/{{$doc->document}}')" data-target="#loaddiff" class="dropdown-item" href="javascript:void(0)" style="text-decoration:none;" role="menuitem">{{$doc->docname}}</a>
                       @endforeach
					   @endif
                      </div>
                    </div>
                     
                      <button onclick="decide('{{$emp->id}}',1,'{{$emp->email}}','{{$emp->name}}','{{$job}}')"  type="button" title="Accept" class="btn btn-icon btn-default btn-outline"><i class="icon  wb-check" aria-hidden="true"></i></button>
					  
                      <button onclick="decide('{{$emp->id}}',2,'{{$emp->email}}','{{$emp->name}}','{{$job}}')" type="button" title="Reject" class="btn btn-icon btn-default btn-outline"><i class="icon  wb-close" aria-hidden="true"></i></button>
                      <button "urlChange('{{url('profile')}}/{{$emp->id}}')" type="button" title="View Profile" class="btn btn-icon btn-default btn-outline"><i class="icon  wb-eye" aria-hidden="true"></i></button>
					  <button onclick="mail('{{$emp->email}}','{{$emp->name}}','$emp->id')" type="button" title="Mail {{$emp->name}}" class="btn btn-icon btn-default btn-outline" data-toggle="modal" data-target="#mailapps"><i class="icon  wb-envelope" aria-hidden="true"></i></button>
                    </div>
					...
					@endforeach
					<tr>
						<th colspan="5">{{ $applicants->render() }}</th>
					</tr>
					@else
					<tr>
						<th colspan="5">
...
@foreach($applicants as $emp)
      <?php $docs=app('App\Repositories\EmployeeRepository')->getdoc($emp->id); ?>
					 ...
					 <object style="width:900px;" data="{{url('upload')}}/{{$doc->document}}" type="application/pdf" data-caption="{{$emp->name}}"></object>
