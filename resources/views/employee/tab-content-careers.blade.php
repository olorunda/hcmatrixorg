...
@else
<h4 class="text-danger no-pilot">{{$direct->name}} has no career aspirations set.</h4>
@endif
...
<div class="row row-lg">
  <div class="col-md-12 col-xs-12">
    <div class="panel panel-bordered" style="margin-bottom: 4px;">
      ...
      <div class="panel-body">
        <div class="panel-group panel-group-simple" id="career{{Auth::user()->id}}" aria-multiselectable="true" role="tablist">
          @foreach($cras as $cra)
          <div class="panel">
            <div class="panel-heading" id="pilotHeadingModal1{{Auth::user()->id}}{{$cra->id}}" role="tab">
              <a class="panel-title" data-toggle="collapse" href="#pilotcollapsModal1{{Auth::user()->id}}{{$cra->id}}" data-parent="#career{{Auth::user()->id}}" aria-expanded="false" aria-controls="pilotcollapsModal1{{Auth::user()->id}}{{$cra->id}}">
                <h5 class="text-warning">Commitment</h5>
              </a>
              <span class="text-default" style="word-wrap: break-word;">
                {{$cra->commitment}}
            ...
            <div class="panel-collapse collapse" id="pilotcollapsModal1{{Auth::user()->id}}{{$cra->id}}" aria-labelledby="pilotHeadingModal1{{Auth::user()->id}}{{$cra->id}}" role="tabpanel">
              <div class="panel-body">
                <div class="row">
                  <div class="col-md-2 col-xs-12">
                    <h5>Objectvie</h5>
                  ...
                  <div class="col-md-10 col-xs-12">
                    <ul>
                      <li>
                        ...
                        <br>
                        <p>
                          <a href="javascript:void(0)" onclick="editComm('cmal1{{$l}}{{Auth::user()->id}}')" id="edit"><i class="icon wb-edit"></i></a>
                          <i class="hide" id="status{{$l}}"></i>
                        </p>
                        <div class="click2edit" id="cmal1{{$l}}{{Auth::user()->id}}" empid="{{Auth::user()->id}}" goalid="{{$cra->id}}" style="word-wrap: break-word;">
                        @if(count($lmcracmt) > 0)
                        @if($lmcracmt->lm_comment != NULL)
                        {{$lmcracmt->lm_comment}}
                        @endif
...
@else
<h4 class="text-danger no-pilot">{{Auth::user()->name}} has no career aspirations set.</h4>
@endif
