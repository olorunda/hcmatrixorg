...
@else
<h4 class="text-danger no-pilot">{{$direct->name}} has no development plans set.</h4>
@endif
...
<div class="row row-lg">
  <div class="col-md-12 col-xs-12">
    <div class="panel panel-bordered" style="margin-bottom: 4px;">
      ...
      <div class="panel-body">
        <div class="panel-group panel-group-simple" id="idp{{Auth::user()->id}}" aria-multiselectable="true" role="tablist">
          @foreach($idpgoals as $idpgoal)
          <div class="panel">
            <div class="panel-heading" id="pilotHeadingModal1{{Auth::user()->id}}{{$idpgoal->id}}" role="tab">
              <a class="panel-title" data-toggle="collapse" href="#pilotcollapsModal1{{Auth::user()->id}}{{$idpgoal->id}}" data-parent="#idp{{Auth::user()->id}}" aria-expanded="false" aria-controls="pilotcollapsModal1{{Auth::user()->id}}{{$idpgoal->id}}">
                <h5 class="text-warning">Commitment</h5>
              </a>
              <span class="text-default" style="word-wrap: break-word;">
                {{$idpgoal->commitment}}
            ...
            <div class="panel-collapse collapse" id="pilotcollapsModal1{{Auth::user()->id}}{{$idpgoal->id}}" aria-labelledby="pilotHeadingModal1{{Auth::user()->id}}{{$idpgoal->id}}" role="tabpanel">
              <div class="panel-body">
                <div class="row">
                  <div class="col-md-2 col-xs-12">
                    <h5>Objectvie</h5>
                  ...
                  <div class="col-md-10 col-xs-12">
                    <ul>
                      <li>
                        ...
                        <br>
                        <p>
                          <a href="javascript:void(0)" onclick="editComm('cmak1{{$k}}{{Auth::user()->id}}')" id="edit"><i class="icon wb-edit"></i></a>
                          <i class="hide" id="status{{$k}}"></i>
                        </p>
                        <div class="click2edit" id="cmak1{{$k}}{{Auth::user()->id}}" empid="{{Auth::user()->id}}" goalid="{{$idpgoal->id}}" style="word-wrap: break-word;">
                          @if(count($lmidcmt) > 0)
                          @if($lmidcmt->lm_comment != NULL)
                          {{$lmidcmt->lm_comment}}
                          @endif
...
@else
<h4 class="text-danger no-pilot">{{Auth::user()->name}} has no development plans set.</h4>
@endif
