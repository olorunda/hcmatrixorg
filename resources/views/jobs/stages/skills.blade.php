	...
	<div class="ribbon ribbon-clip ribbon-reverse ribbon-success" id="skillribbon">
		...
		<div id="skills">
			...
			<form autocomplete="off">
				<input type="hidden" name="_skill_csrf" id="_skill_csrf" value="{{csrf_token()}}">
				<div class="form-group">
					...
					<select class="form-control" required="required" data-plugin="select2" data-allow-clear="true" id="skillset">
						...
						<option value="{{$jobskill->id}}">{{$jobskill->skill}}</option>
						@endforeach
						@else
				...
				<div class="form-group">
					...
					<select class="form-control" required="required" data-plugin="select2" data-allow-clear="true" id="compset">
						...
						<option value="{{$competence->id}}">{{$competence->proficiency}}</option>
						@endforeach
						@else
			...
			<br><br>
			<form autocomplete="off">
				<input type="hidden" name="_csrf_train_token" id="_csrf_train_token" value="{{csrf_token()}}">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="row row-lg">
							<div class="col-md-12" style="display: none;" id="skilledited{{$skill->id}}">
								<div class="col-md-12">
							...
							<div class="col-md-12" style="display: none;" id="skilleditederr{{$skill->id}}">
								<div class="col-md-12">
							...
							<div class="col-md-12">
								<div class="col-md-6">
									<div class="form-group">
										<label class="control-label">Skill</label>
										<select class="form-control" required="required" data-plugin="select2" data-allow-clear="true" id="edit_skillset{{$skill->id}}">
											@foreach($jobskills as $jobskill)
											@if($skill->skill_id == $jobskill->id)
											<option selected="selected" value="{{$jobskill->id}}">{{$jobskill->skill}}</option>
											@else
											<option value="{{$jobskill->id}}">{{$jobskill->skill}}</option>
											@endif
											@endforeach
								...
								<div class="col-md-6">
									<div class="form-group">
										<label class="control-label">Competence Level</label>
										<select class="form-control" required="required" data-plugin="select2" data-allow-clear="true" id="edit_compset{{$skill->id}}">
											@foreach($competencies as $competence)
											@if($skill->proficiency_id == $competence->id)
											<option selected="selected" value="{{$competence->id}}">{{$competence->proficiency}}</option>
											@else
											<option value="{{$competence->id}}">{{$competence->proficiency}}</option>
											@endif
											@endforeach
								...
								<div class="col-md-12">
									<button type="button" class="btn btn-success m-b-5" onclick="updateSkill({{$skill->id}})"><i class="icon wb-check-circle"></i> </button>
									<button type="button" class="btn btn-danger" onclick="deleteSkill({{$skill->id}})"><i class="icon ion-ios-trash"></i></button>
								</div>
							</div>
						</div>
						@endforeach
						...
						<div class="panel-group" id="trainingaccordion" aria-multiselectable="true" role="tablist">
							...
							<div class="panel">
								<div class="panel-heading" id="$trainEd{{$training->id}}" role="tab">
									<a class="panel-title" data-toggle="collapse" href="#trainingEdData{{$training->id}}" data-parent="#trainingaccordion" aria-expanded="false" aria-controls="trainingEdData{$ed->id}}">
										{{ $training->training_name }}
									</a>
								</div>
							</div>
							<div class="panel-collapse collapse" id="trainingEdData{{$training->id}}" aria-labelledby="trainEd{{$training->id}}" role="tabpanel">
								<div class="panel-body">
									<div class="row row-lg">
										<div class="col-md-12" style="display: none;" id="trainedited{{$training->id}}">
											<div class="col-md-12">
												<div class="alert alert-success alert-dismissible">
													<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
													<strong>Success! </strong> Record Updated.
										...
										<div class="col-md-12" style="display: none;" id="traineditederr{{$training->id}}">
											<div class="col-md-12">
												<div class="alert alert-danger alert-dismissible">
													<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
													<strong>Failed! </strong> Record was not updated. Please try again.
										...
										<div class="col-md-12">
											<div class="col-md-12">
												<div class="form-group form-material floating" data-plugin="formMaterial">
													<input type="text" class="form-control" name="train_name" id="edit_train_name{{$training->id}}" value="{{$training->training_name}}">
													<label class="floating-label">Name of Training</label>
												</div>
											</div>
										</div>
										<div class="col-md-12">
											<div class="col-md-6">
												<div class="form-group form-material floating" data-plugin="formMaterial">
													<input type="text" class="form-control" name="train_start" id="edit_train_start{{$training->id}}" data-plugin="datepicker" data-date-format="yyyy-mm-dd" value="{{$training->start_date}}">
													<label class="floating-label">Start Date</label>
												</div>
											</div>
											<div class="col-md-6">
												<div class="form-group form-material floating" data-plugin="formMaterial">
													<input type="text" class="form-control" name="edit_train_end" id="edit_train_end{{$training->id}}" data-plugin="datepicker" data-date-format="yyyy-mm-dd" value="{{$training->end_date}}">
													<label class="floating-label">End Date</label>
												</div>
											</div>
										</div>
										<div class="col-md-12">
											<div class="col-md-6">
												<div class="form-group form-material floating" data-plugin="formMaterial">
													<input type="text" class="form-control" name="edit_train_inst" id="edit_train_inst{{$training->id}}" value="{{$training->institution}}">
													<label class="floating-label">Institution</label>
												</div>
											</div>
											<div class="col-md-6">
												<div class="form-group form-material floating" data-plugin="formMaterial">
													<input type="text" class="form-control" name="edit_train_loc" id="edit_train_loc{{$training->id}}" value="{{$training->location}}">
													<label class="floating-label">Location</label>
												</div>
											</div>
										</div>
										<div class="col-md-12">
											<button type="button" class="btn btn-success m-b-5" onclick="updateTrain({{$training->id}})"><i class="icon wb-check-circle"></i> Save changes</button>
											<button type="button" class="btn btn-danger" onclick="deleteTrain({{$training->id}})"><i class="icon ion-ios-trash"></i> Delete</button>
										</div>
									</div>
								</div>
							</div>
