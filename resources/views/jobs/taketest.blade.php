	...
	<script>
	function complete(){
		$.get('{{url('completed')}}?jobid={{$_GET['job_id']}}',function(data,status,xhr){
...
function loadtimer(){
 ...
 $(function(){
	...
	sessionStorage.setItem('count',0);
	 ...
	 @if(session()->has('on')|| session('on')==1)
		 loadtimer();
	...
	getquestion(sessionStorage.getItem('count'));
	 ...
	 $(window).on('beforeunload',function(){
		...
		$.get('{{url('seton')}}',function(data,status,xhr){
	...
	//getquestioncount
	$.get('{{url('count')}}',function(data,status,xhr){
 ...
 function getquestion(num){	
	 if(num==0){
		...
		$.get('{{url('taketest')}}/'+num,function(data,status,xhr){
		console.log(data);
	...
	$.get('{{url('applicant')}}/submittest/'+userid+'/'+questionid+'/'+selectedoption+'/{{$jobid}}',function(data,status,xhr){
  ...
  <div class="modal-dialog modal-lg modal-warning ">
    ...
    <div class="modal-content">
	  ...
	  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
       ...
       <!-- <button type="button" class="close" data-dismiss="modal">&times;</button> -->
        <h4 class="modal-title">	@if($checktesttake==0)	
	...
	@if($checktesttake==0)	
		<div id="refreshclock">
			...
			</div><!-- <button style=" margin:0 0 0 45%;" id="ref" class="btn btn-success"><i class="fa fa-refresh"></i>Refresh Timer</button><br>--> @endif
	<div class="row" style="">
			...
			<div class="panel panel-bordered" >
				...
				<h4><b>Your Score is :{{$getscore}}</b></h4>
