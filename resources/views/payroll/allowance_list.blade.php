@include('layouts.header', ['page_title' => 'Salary component List'])
  <!-- Page -->
  <div class="page">
                ...
                <div class="card-block p-0">
                  ...
                  <table class="table table-hover tabletable-striped dataTable" id="data_table">
                      ...
                      <tbody> 
                            ...
                            <tr>                                
                                <td>{{$sno++}} </td>
                                <td>@if($allowance->job_role==Config::get('constants.roles.Admin_User')) {{'Admin'}} @endif
                                    @if($allowance->job_role==Config::get('constants.roles.People_Manager')) {{'People Manager'}} @endif
                                    @if($allowance->job_role==Config::get('constants.roles.Employee')) {{'Employee'}} @endif
                                    @if($allowance->job_role==Config::get('constants.roles.Doctor')) {{ 'Doctor'}} @endif
                                    @if($allowance->job_role==Config::get('constants.roles.Factory_Employee')) {{ 'Factory Employee'}} @endif</td>
                                <td>{{ $allowance->name }}</td>
                                <td>@if($allowance->is_allowance==1) {{ 'Allowance' }} @else {{ 'Deduction' }} @endif</td>
                                <td>@if($allowance->is_formula==0){{ $allowance->charge_percentage }} @endif</td>
                                <td>{{ $allowance->charge_formula }}</td>
                                <td class="actions" id="status_div_{{ $allowance->id}}">@if($allowance->status==0) <aonclick="fnStatusChange({{ $allowance->id}},{{ $allowance->status}}, {{$allowance->is_allowance}})"><i class="btn btn-sm btn-warning waves-effect icon fa-eye-slash" aria-hidden="true"   title="Make Active" ></i></a> @else <a onclick="fnStatusChange({{ $allowance->id}},{{ $allowance->status}}, {{$allowance->is_allowance}})"><i class="btn btn-sm btn-success waves-effect icon fa-eye" aria-hidden="true" title="Make Inactive"></i></a> @endif</td>
                                <td class="actions"><a onClick="fnEditAllowance({{$allowance->id}})"><i class="btn btn-sm btn-primary waves-effect icon fa-edit" aria-hidden="true" title="Edit"></i></a><!--<a onClick="fnDelete({{ $allowance->id}}, {{$allowance->is_allowance}})"><i class="btn btn-sm btn-danger waves-effect icon fa-close" aria-hidden="true" title="Delete"></i></a>--></td>
                            </tr>                            
                            @endforeach 
                      </tbody> 
                  </table>           
          ...
          <div class="row row-lg col-xs-12">            
            <div class="col-xs-12">
                  ...
                  <option value = ''>Select</option>
                    @foreach($roles as $role)  
                      @if($role->role!='' && $role->role!=0)
                        <option value = "{{ $role->role}}" {{ ((old("job_role")!='' && old("job_role") ==  $role->role) || (isset($allowance) && $allowance->job_role!='' && $allowance->job_role ==  $role->role)) ? "selected=selected":"" }}>
              ...
              <div class="form-group">
                ...
                <input type="radio" id="allowance" name="is_allowance" value="1" {{ ((old("is_allowance")!='' && old("is_allowance") ==  1) || (isset($allowance_details) && $allowance_details->is_allowance!='' && $allowance_details->is_allowance ==  1)) ? "checked=checked":"" }}>&nbsp;Allowance&nbsp;&nbsp;
                <input type="radio" id="deduction" name="is_allowance" value="0" {{ ((old("is_allowance")!='' && old("is_allowance") ==  0) || (isset($allowance_details) && $allowance_details->is_allowance!='' && $allowance_details->is_allowance ==  0)) ? "checked=checked":"" }}>&nbsp;Deduction
              </div>
              <div id="is_allowance_err"></div>

              <div class="form-group">
                <label class="example-title">Charge Name&nbsp;<span class="required_filed">*</span></label>
                <input type="text" class="form-control" id="name" name="name" placeholder="Charge Name" value="{{ old('name')!='' ? old('name') : ((isset($allowance_details) && $allowance_details->name!='') ? $allowance_details->name : '') }}">               
              </div>
              <div id="name_err"></div> 
              <div class="form-group">
                <label class="example-title">Percentage / Formula&nbsp;<span class="required_filed">*</span></label>&nbsp;&nbsp;&nbsp;&nbsp;
                <input type="radio" id="chrg_percentage" name="is_formula" onclick="fnChangeCharge(this.value)" value="0" {{ ((old("is_formula")=='' || old("is_formula") ==  0) || (isset($allowance_details) && ($allowance_details->is_formula=='' || $allowance_details->is_formula ==  0))) ? "checked=checked":"" }}>&nbsp;Percentage&nbsp;&nbsp;
                <input type="radio" id="chrg_formula" name="is_formula" onclick="fnChangeCharge(this.value)" value="1" {{ ((old("is_formula")!='' && old("is_formula") ==  1) || (isset($allowance_details) && $allowance_details->is_formula!='' && $allowance_details->is_formula ==  1)) ? "checked=checked":"" }}>&nbsp;Formula                
              </div>
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="charge_percentage" name="charge_percentage" placeholder="Charge in Percentage" value="{{ old('charge_percentage')!='' ? old('charge_percentage') : ((isset($allowance_details) && $allowance_details->charge_percentage!='') ? $allowance_details->charge_percentage : '') }}">               
              </div>
            ...
            <div id="charge_formula_div" style="display:none;">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="charge_formula" name="charge_formula" placeholder="Charge Formula" value="{{ old('charge_formula')!='' ? old('charge_formula') : ((isset($allowance_details) && $allowance_details->charge_formula!='') ? $allowance_details->charge_formula : '') }}">               
              </div>
