...
/*
    ...
    public function basicpay_update(Request $request){ 
        ...
        if($action_button == "add"){
            DB::table(Config::get('constants.tables.BASICPAY'))->insert(
                ['emp_id' => $employee_id, 'emp_num' => $employee_num, 'basicpay' => $basicpay]
            );
    ...
    public function payroll_list()
    {
        ...
        $employees = DB::table($tbl_user)
            ->select("$tbl_user.*", "$tbl_basicpay.basicpay", "$tbl_payroll.id as payroll_id", "$tbl_payroll.ps_issued", "$tbl_payroll.ps_file")
            ->join($tbl_basicpay, "$tbl_user.id", '=', "$tbl_basicpay.emp_id")
    ...
    public function emp_payroll_list()
    {
        ...
        $employees = DB::table($tbl_basicpay)
            ...
            ->join($tbl_user, $tbl_user.'.id', '=', $tbl_basicpay.'.emp_id')
...
//Getting the total no. of avaialble leaves per month
        ...
        $allowances_value = (float)(($basic_pay * $allowances_total) / 100);
        ...
        $deductions_value = (float)(($basic_pay * $deductions_total) / 100);
    ...
    public function fill_payroll_view($id)
    {
        ...
        foreach($payroll_details as $details) {
            if($details->type==1) {
                ...
                $payroll_display .= '<td style="text-align:right">(+) '.number_format(($payroll[0]->basic_pay * $details->percentage)/100,2) .'</td>';       
                $payroll_display .= '</tr>';
            }
        }
    ...
    public function payroll_update(Request $request){ 
        ...
        foreach($allowances_query as $allowance) {
            ...
            DB::table(Config::get('constants.tables.PAYROLL_DETAILS'))->insert(
                ['payroll_id' => $payroll_id, 'type' => $allowance->is_allowance, 'allowance_id' => $allowance->id, 'name' => $allowance->name, 'percentage' => $allowance->charge_percentage]
            );
    ...
    public function create_ps($id)
    {
       ...
       $tbl_settings = Config::get('constants.tables.SETTINGS');
        ...
        $mc_file_uploaded = $pdf->loadHTML($view)->save(public_path('psc').'/'.$ps_file);
        if($mc_file_uploaded)
        {
...
/***************** Allowances / Deductions start **************************/
    ...
    public function view_emp_daily_attendance_calendar(Request $request)
    {
        $id = $request->input('emp_id');
            ...
            $daily_attendances = DB::select("SELECT * FROM ".Config::get('constants.tables.DAILY_ATTENDANCE')." WHERE (Month(date) = ".date('m', strtotime(' -1 month'))." OR Month(date) = ".date('m').")  AND emp_id=".$id . " ORDER BY id ASC");         
      //echo '<pre>'; print_r($daily_attendances); exit();
        ...
        $res_arr = array();
            ...
            {   
              ...
              } else if(!empty($daily_attendances) && $strt == $daily_attendances[$j]->date) {
